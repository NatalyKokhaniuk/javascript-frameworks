...
class App {
    constructor() {
        ...
        this.displayUsers = () => {
            ...
            if (usersListElement) {
                usersListElement.innerHTML = this.userService
                    .getAllUsers()
                    .map((user) => `<li class="d-flex justify-content-between align-items-center mb-2">${user.name} (${user.email}) - ID: ${user.id} - Позичені книги: ${user.borrowedBooks.length}
          </li>`)
                    .join("");
            }
        };
        ...
        this.showBorrowBookModal = (bookId) => {
            Modal.showBorrowBookModal(bookId, (userId) => this.borrowBook(bookId, userId));
        };
        this.deleteBook = (bookId) => {
